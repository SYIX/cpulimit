...
- 目标进程无权限或不存在:cpulimit `-z` 对这类 pid 会将提示打到 stdout 并以退出码 0 退出,GUI 无法从 stderr/退出码感知;因此 GUI 在启动子进程前用 `kill(pid, 0)` 预检,失败时直接在任务行显示"进程不存在或无权限"并在 5 秒后移除。stderr 捕获保留,作为异常退出(非零退出码)的兜底。
